...
class BlueBoatFeatureExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: Box, hist_frame: int = 50, imu_size: int = 10, action_size: int = 6, cmd_size: int = 6, latent_dim: int = 32):
        super(BlueBoatFeatureExtractor, self).__init__(observation_space, features_dim=latent_dim)
        ...
        self.fc = nn.Sequential(
            nn.Linear(imu_flattened_dim + action_flattened_dim + self.cmd_length, self.latent_dim),
            nn.ReLU(),
