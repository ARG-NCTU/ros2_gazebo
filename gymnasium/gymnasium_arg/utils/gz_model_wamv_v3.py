...
class WAMVV3_GZ_MODEL(GZ_MODEL):

        
    def __init__(self, world, name, path, pose: Pose, info={'veh':'wamv_v3', 'maxstep': 4096, 'max_angular': np.pi/4, 'max_thrust': 15*746/9.8, 'hist_frame': 5}):
        ...
        self.info['max_angular'] = info.get('max_angular', np.pi/4)
        ...
        self.pub['right_ang'] = self.create_publisher(Float64, f'/{self.name}/joint/right/thruster/cmd_pos', 1)
        self.pub['left_ang'] = self.create_publisher(Float64, f'/{self.name}/joint/left/thruster/cmd_pos', 1)
        self.pub['right_thrust'] = self.create_publisher(Float64, f'/{self.name}/joint/right/thruster/cmd_thrust', 1)
        self.pub['left_thrust'] = self.create_publisher(Float64, f'/{self.name}/joint/left/thruster/cmd_thrust', 1)
        ...
        self.obs['action'] = np.zeros((self.info['hist_frame'], 4))
    ...
    def step(self, action:np.ndarray):
        ...
        self.obs['action'][0] = action
        self.info['step_cnt'] += 1
        left_thrust = Float64()

        left_thrust.data = action[0]*self.info['max_thrust']
        right_thrust = Float64()
        right_thrust.data = action[1]*self.info['max_thrust']
        left_ang = Float64()
        left_ang.data = action[2]*self.info['max_angular']
        right_ang = Float64()
        right_ang.data = action[3]*self.info['max_angular']

        self.pub['left_thrust'].publish(left_thrust)
        self.pub['right_thrust'].publish(right_thrust)
        self.pub['left_ang'].publish(left_ang)
        self.pub['right_ang'].publish(right_ang)
