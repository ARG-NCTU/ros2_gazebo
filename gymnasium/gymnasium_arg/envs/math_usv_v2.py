...
class MATH_USV_V2(gym.Env):
    ...
    def __init__(self, render_mode="none", hist_frame=50, seed=0, device='cpu'):
        super(MATH_USV_V2, self).__init__()
        ...
        self.cmd_vel = torch.zeros(2, device=self.device)
    ...
    def step(self, action):
        ...
        rew1 = k1*(2*torch.exp(-(self.cmd_vel[0]**2/0.25 + self.cmd_vel[1]**2/0.25))-1)
        ...
        output = "\rstep:{:4d}, cmd: [x:{}, y:{}, yaw:{}], action: [l_t:{}, r_t:{}, l_a:{}, r_a:{}], rews: [{}, {}, {}] const:[{}]".format(
            ...
            " {:4.2f}".format(self.refer_pose[2].item()) if sgn_bool(self.refer_pose[2].item()) else "{:4.2f}".format(self.refer_pose[2].item()),
    ...
    def get_observation(self):
        ...
        yaw = self.refer_pose[2].item()-y
        # yaw = {-np.pi~np.pi}
        yaw = (yaw + np.pi) % (2 * np.pi) - np.pi
        imu_obs = np.hstack((np.array([r]).T, np.array([p]).T, np.array([yaw]).T, imu_obs[:, 4:])) 
