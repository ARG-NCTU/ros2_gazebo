...
class Multi_BlueBoat_V1(gym.Env, Node):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.__unpause()
        return self.get_observation(np.zeros((self.info['num_envs'], 6))), self.info
    ...
    def step(self, actions):
        ...
        if actions.shape[0] != self.info['num_envs']:
            cmd_vel.twist.linear.x = float(actions[0])
            cmd_vel.twist.linear.y = float(actions[1])
            cmd_vel.twist.linear.z = float(actions[2])
            cmd_vel.twist.angular.x = float(actions[3])
            cmd_vel.twist.angular.y = float(actions[4])
            cmd_vel.twist.angular.z = float(actions[5])
