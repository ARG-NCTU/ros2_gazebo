...
class LunarLander(gym.Env, EzPickle):
    ...
    def reset(
        ...
        *,
        ...
        if self.render_mode == "human":
            self.render()
        return self.step(np.array([0, 0]) if self.continuous else 0)[0], self.info
    ...
    def step(self, action):
        ...
        if self.render_mode == "human":
            self.render()
        return np.array(state, dtype=np.float32), reward, terminated, False, self.info
